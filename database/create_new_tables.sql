-- Script para crear solo las nuevas tablas si no existen
...
SELECT 'fixed_expenses table created' as status WHERE EXISTS (
  SELECT 1 FROM information_schema.tables 
  WHERE table_name = 'fixed_expenses'
);

SELECT 'accounts_receivable table created' as status WHERE EXISTS (
  SELECT 1 FROM information_schema.tables 
